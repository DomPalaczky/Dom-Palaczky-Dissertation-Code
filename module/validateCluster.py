...
def plotFeatureImportance(columnnames, featureImportance, path, overall = True, clusnum = None, show=False): 
    #create directory
    try:
...
def plotFeatureImportanceByClass(columnnames, featureImportance, X, labels, clas, path, show = False): #uses dtreeviz
    #create directory
    try:
        os.mkdir(path + '/feature_importance')
    except:
    ...
    for i in range(max(labels)+1):
        ...
        plotFeatureImportance(columnnames, featureImportance, path, overall = False, clusnum = i, show = show)

def treevis(clas,X,labels,columnnames, path, show=False):
...
def tsneScatter2D(X,labels,path, title=None, show = False):
...
def pcaScatter(X, labels, path, title = None, show = False):
